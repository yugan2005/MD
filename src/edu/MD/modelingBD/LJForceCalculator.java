...
public class LJForceCalculator {
	...
	private final BigDecimal sigma6, sigma12, epsilon, cutoffPotential;

	private LJForceCalculator(final BigDecimal sigma6, final BigDecimal sigma12, final BigDecimal epsilon,
			final BigDecimal cutoffPotential) {
		this.sigma6 = sigma6;
	...
	public MDVector calculate(MDVector p1_p2) {
		BigDecimal norm = p1_p2.norm();
		BigDecimal coefficient = (new BigDecimal(48)).multiply(epsilon).multiply(sigma12).divide(norm.pow(14)).subtract((new BigDecimal(24).multiply(epsilon).multiply(sigma6).divide(norm.pow(8)))).add(cutoffPotential.divide(norm));
